...
// Disable Server-Side Rendering (SSR) for browser-only document parsing libraries
const DocConverter = dynamic(
  () => import("../../../components/tools/DocConverter").then((mod) => mod.DocConverter),
  { ssr: false }
);
      ...
      <div className="flex-1 flex flex-col justify-center items-center max-w-4xl mx-auto w-full px-4 pt-32 pb-12 relative z-10">
        ...
        <div className="text-center space-y-3 mb-8 w-full">
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-zinc-100 leading-tight font-display">
            Chuyển Đổi <span className="text-[#ff5e3a]">Tài Liệu</span>
          </h1>
          <p className="text-xs md:text-sm text-zinc-400 leading-relaxed max-w-md mx-auto">
            Chuyển đổi Markdown sang Word, Word sang Markdown hoặc trích xuất chữ từ PDF trực tiếp 100% trên trình duyệt.
          </p>
        </div>

        {/* Dynamic Tool Card */}
        <ToolCard
          title="Chuyển đổi tài liệu đa văn bản"
          description="Nạp tài liệu Markdown hoặc tải lên file Word/PDF để chuyển đổi định dạng và sao chép văn bản mong muốn."
        >
          <DocConverter />
        </ToolCard>
