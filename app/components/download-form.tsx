...
export function DownloadForm() {
  ...
  return (
    <div className={`glass-panel border border-[#232332] bg-[#161622] rounded-2xl relative overflow-hidden transition-all duration-500 shadow-[0_30px_100px_rgba(0,0,0,0.6)] hover-scale-effect ${
      showPreviewColumn ? "max-w-4xl w-full" : "max-w-xl w-full"
    }`}>
      {/* Repeating Film Reel Sprockets decoration along the card border (Signature) */}
              ...
              <div className="space-y-2">
                ...
                <div className="relative flex items-center">
                  <input
                    ...
                    disabled={isLoading}
                    className="w-full bg-[#0d0d12]/80 border border-zinc-800/80 focus:border-[#ff5e3a]/80 focus:ring-1 focus:ring-[#ff5e3a]/20 text-zinc-100 rounded-xl px-4 py-3.5 pr-24 text-sm placeholder:text-zinc-600 outline-none transition-all duration-200"
                    required
                  />
                  <button
                    type="button"
                    onClick={handlePaste}
                    disabled={isLoading}
                    className="absolute right-3 px-3 py-1.5 bg-zinc-850 hover:bg-zinc-800 border border-zinc-700 text-zinc-300 rounded-lg text-xs font-medium flex items-center gap-1.5 transition-all outline-none active-press"
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <select
                    ...
                    disabled={isLoading}
                    className="w-full bg-[#0d0d12]/80 border border-zinc-800/80 focus:border-[#ff5e3a]/80 focus:ring-1 focus:ring-[#ff5e3a]/20 text-zinc-100 rounded-xl px-4 py-3.5 pr-10 text-sm outline-none transition-all duration-200 appearance-none cursor-pointer"
                  >
          ...
          {showPreviewColumn && (
            <div className="md:col-span-5 space-y-5 flex flex-col justify-between h-full border-t md:border-t-0 md:border-l border-zinc-800/80 pt-6 md:pt-0 md:pl-6">
              <div>
                ...
                {/* 35mm Film Frame Cell Wrapper */}
                <div className="sprocket-border relative overflow-hidden bg-zinc-950 border border-zinc-800/85">
                  <div className="film-sprockets-top" />
                  
                  <div className="relative aspect-video w-full flex items-center justify-center bg-zinc-900 group/thumb">
                    {youtubeId ? (
                      <img
                      ...
                      <div className="flex flex-col items-center gap-2 text-zinc-600">
                        ...
                        <span className="text-[10px]">Đang chờ tải liên kết...</span>
                      </div>
                    )}
      ...
      {/* Repeating Film Reel Sprockets decoration at the card bottom (Signature) */}
      <div className="film-sprockets-bottom" />
    </div>
  );
}
